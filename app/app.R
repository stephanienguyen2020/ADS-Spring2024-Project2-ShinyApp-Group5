...
rename(state = `state&teritory`, longitude = `longitude`, latitude = `latitude`)
...
ui <- fluidPage(
  ...
  navbarPage(title = "",
    ...
    tabPanel("Geographical Overview", fluidPage(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          selectInput("year", "Select Year of Loss:",
                      choices = c("All Years", unique(sort(df$fyDeclared)))),
          selectInput("state", "Select State:",
                      choices = c("All States", unique(sort(df$state))))
        ),
        mainPanel(
          leafletOutput("map")
        )
    ...
    tabPanel("Conclusions",
             # Background image with reduced opacity
             tags$div(style = "position: relative; text-align: center; color: white;",
                      tags$img(src = "https://www.economist.com/cdn-cgi/image/width=1424,quality=80,format=auto/content-assets/images/20230617_AMP502.jpg", style = "opacity: 0.90; width: 100%;"),
                      # White box with text, centered over the image
                      tags$div(style = "position: absolute; top: 40%; left: 25%; right: 25%; background-color: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);",
                               tags$h3("Agricultural Resilience and Insurance Solutions", style = "color: black;"),
                               tags$p("For the agricultural sector, where timing and decision-making are closely tied to the rhythms of nature and the unpredictability of environmental factors, our app are not just tools but essential partners in planning and resilience. Our predictive modeling based on historical and real-time data represents a pivotal advancement in the management of fire incidents, offering a strategic edge in forecasting and mitigating risks.", style = "text-align: justify; color: black;"),
                               tags$p("By assessing the fire risk in different periods, farmers can make informed decisions on what crops to grow when, enhancing their chances of a successful harvest. More importantly, farmers can use this data to decide on the level of insurance coverage necessary to protect their investments, ensuring they are neither over-insured nor under-insured.", style = "text-align: justify; color: black;"),
                               tags$p("Our app provides more than just operational benefits; it offers comprehensive support that recognizes the importance of agriculture to national security and food supply. In an era where climate change increases the frequency and severity of fire incidents, protecting agriculture is paramount. Our service not only helps individual farmers manage risk more effectively but also contributes to the broader objective of securing food supplies and reinforcing the resilience of the agricultural sector against environmental threats.", style = "text-align: justify; color: black;")
                      )
             )
    ),
    #################### Tab 9: Reference ####################
    tabPanel("References" ,
             ...
             tags$a(
               ...
               "US Disaster Declarations Summaries", 
             ),br(),
             tags$a(
               href = "https://www.fema.gov/openfema-data-page/fema-web-disaster-declarations-v1",
               "FEMA Web Disaster Declarations", 
             ),br(),
             tags$a(
               href = "https://www.fema.gov/openfema-data-page/fima-nfip-redacted-claims-v2",
               "FIMA NFIP Redacted Claims", 
             ),br(),
             tags$a(
               href = "https://www.fema.gov/openfema-data-page/fema-web-disaster-summaries-v1",
               "FEMA Web Disaster Summeries"
             ),br(),
             tags$a(
             ...
             tags$a(
               href = "https://github.com/stephanienguyen2020/ADS-Spring2024-Project2-ShinyApp-Group5",
               "https://github.com/stephanienguyen2020/ADS-Spring2024-Project2-ShinyApp-Group5"
             )
          )
  )
)
...
server <- function(input, output) {
  ...
  output$map <- renderLeaflet({
    ...
    # Choose the data based on the display choice and add markers
    if(input$display == "geo") {
      # For "Geographical Location of Fire Incidents", use state level aggregation
      data_by_state <- data_filtered %>%
        group_by(state) %>%
        summarise(count = n(), .groups = 'drop') %>%
        left_join(state_coords, by = "state")
      
      m <- m %>%
        addCircleMarkers(data = data_by_state, ~longitude, ~latitude,
                         popup = ~paste("State:", state, "<br>",
                                        "Number of Incidents:", count),
                         clusterOptions = cluster_options,
                         color = "red", fillOpacity = 0.8, radius = 8)
    } else {
      # For "Fire Incident Types", use declarationTitle level aggregation within each state
      data_by_declarationTitle <- data_filtered %>%
        group_by(state, declarationTitle) %>%
        summarise(count = n(), .groups = 'drop') %>%
        left_join(state_coords, by = "state")
      
      m <- m %>%
        addCircleMarkers(data = data_by_declarationTitle, ~longitude, ~latitude,
                         popup = ~paste("Title:", declarationTitle, "<br>",
                                        "State:", state, "<br>",
                                        "Number of Incidents:", count),
                         clusterOptions = cluster_options,
                         color = "blue", fillOpacity = 0.8, radius = 8)
    }
